...
async function scan(query, category, code, cap) {
  ...
  const report = { ...query, category, total, imported: seen.size, complete: total !== null && seen.size === total, checkedAt: new Date().toISOString() };
...
async function exportData() {
  ...
  const history = db.prepare('SELECT record FROM scans ORDER BY id').all().map(row => JSON.parse(row.record));
  ...
  const result = { generatedAt: new Date().toISOString(), coverage: 'partial', scans, records };
