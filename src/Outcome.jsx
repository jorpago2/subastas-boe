...
export default function Outcome({ row, compact = false }) {
  ...
  if (!active && row.status !== 'Pasada') return null;
  const result = row.certifiedOutcome || row.authenticatedOutcome || row.outcome;
  const label = !result ? 'Pendiente de consultar' : result.status === 'Con pujas' && Number.isSafeInteger(result.highestBid) ? `${active ? 'Puja máxima actual' : 'Puja máxima'}: ${amount(result.highestBid)}` : result.status === 'Por lotes' ? `${active ? 'Pujas actuales' : 'Resultado por lotes'}` : result.status === 'Sin pujas' ? `${active ? 'Sin pujas registradas' : 'Sin pujas'}` : result.status === 'Cancelada' ? 'Subasta cancelada' : 'Pujas: dato no publicado';
  if (compact) return <p className="outcome-summary">{label}</p>;
  return <section className="outcome" aria-label={active ? 'Pujas actuales' : 'Resultado de la subasta'}>
    <h4>{active ? 'Pujas actuales' : 'Cómo terminó'}</h4><p>{label}</p>
    ...
    <p className="outcome-note">{active ? 'La puja máxima puede cambiar hasta el cierre y no acredita una adjudicación.' : 'Adjudicación definitiva: no confirmada en los datos recuperados. La puja máxima no acredita el precio de adjudicación.'}</p>
