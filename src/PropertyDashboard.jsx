...
export default function PropertyDashboard({ row, currency, date, factValue }) {
  ...
  return <section className="property-dashboard" aria-label="Resumen del inmueble">
    ...
    <div className="dashboard-situations">{situations.map(([key, label, note]) => <div key={key}><h5>{label}</h5><Tag type={facts?.[key] === 'occupied' || facts?.[key] === 'present' ? 'warm-gray' : 'gray'}>{factValue(facts, key)}</Tag><p>{note}</p></div>)}</div>
